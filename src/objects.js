...
SpriteMorph.prototype.blockColor = {
    ...
    mod: new Color(106, 28, 232),
    popups: new Color(194, 10, 77),
    blocks: new Color(242, 97, 7),
    debugging: new Color(247, 56, 22)
};
...
SpriteMorph.prototype.initBlocks = function () {
    ...
    */
        ...
        repeatingList: {
            ...
            type: 'reporter',
            category: 'lists',
            spec: 'list of %s repeated %n times',
            defaults: ['', '3']
        },
        ...
        doSendNetRequest: {
            ...
            type: 'reporter',
            category: 'mod',
            spec: 'request %netRequest to %txt with %s and headers %netHeader',
            defaults: ['GET', 'https://www.github.com']
        },
        promptCreateBlock: {
            modded: true,
            type: 'command',
            category: 'mod',
            spec: 'prompt create block'
        },
        ...
        doShowPopup: {
            ...
            type: 'command',
            category: 'popups',
            spec: 'show popup %title %body',
            defaults: ['Notice', 'Snap:Modular block popup']
        },
        doShowErrorPopup: {
            modded: true,
            type: 'command',
            category: 'popups',
            spec: 'show error popup %body',
            defaults: ['Uh uh, something broke!']
        },
        lastPromptAnswer: {
            modded: true,
            type: 'reporter',
            category: 'popups',
            spec: 'last answer for popup'
        },
        hasLastPromptAnswer: {
            type: 'predicate',
            modded: true,
            category: 'popups',
            spec: 'has last answer for popup'
        },
        doPromptYesOrNo: {
            modded: true,
            type: 'command',
            category: 'popups',
            spec: 'ask yes or no question %title %body',
            defaults: ['Question', 'Pick an answer.']
        },
        doPromptYesOrNoCustomButtons: {
            modded: true,
            type: 'command',
            category: 'popups',
            spec: 'ask yes or no question %title %body %br yes button labeled %txt %br no button labeled %txt',
            defaults: ['Question', 'Pick an answer.', 'Accept', 'Deny']
        },
        doPromptText: {
            modded: true,
            type: 'command',
            category: 'popups',
            spec: 'prompt for text %title default value %txt',
            defaults: ['Question', '']
        },
        ...
        doWarnToConsole: {
            ...
            type: 'command',
            category: 'debugging',
            spec: 'warn %txt to console',
            defaults: ['Warning']
        }
    };
};

SpriteMorph.prototype.repeatingList = function(item, amount) {
    var value = [];
    var shouldCallValue = typeof(item) === 'function';
    for(var i = 0; i < amount; i++) {
        value[i] = shouldCallValue ? item() : item;
    }
...
SpriteMorph.prototype.doLogToConsole = function(text) {
    console.log(text);
}
SpriteMorph.prototype.doWarnToConsole = function(text) {
    console.warn('Snap! warning:\n' + text);
}

SpriteMorph.prototype.doSendNetRequest = function(type, domain, payload, headers) {
    return SnapExtensions.primitives.get('xhr_request(mth, url, dta, hdrs)')(type, domain, payload, headers);
}

SpriteMorph.prototype.getAllBlocks = function() {
    return new List(
...
SpriteMorph.prototype.lastPromptAnswer = function() {
...
SpriteMorph.prototype.doShowPopup = function(title, body) {
    // this.parentThatIsA(IDE_Morph).inform(title, body);
    var questionDialog = new DialogBoxMorph();
    questionDialog.inform(title, body, world, null);
};
SpriteMorph.prototype.doShowErrorPopup = function(body) {
    // this.parentThatIsA(IDE_Morph).inform(title, body);
    var questionDialog = new DialogBoxMorph();
    questionDialog.inform("Error", body, world, null);
};
...
SpriteMorph.prototype.blockAlternatives = {
    ...
    // popups:
    doShowPopup: ['doPromptYesOrNo', 'doPromptYesOrNoCustomButtons'],
    doPromptYesOrNo: ['doShowPopup', 'doPromptYesOrNoCustomButtons'],
    doPromptYesOrNoCustomButtons: ['doPromptYesOrNo'],
...
SpriteMorph.prototype.blockTemplates = function (
    ...
    const modCategories = ['popups', 'blocks', 'debugging'];
    ...
    if (category === 'motion') {
        ...
        blocks.push('-');
        blocks.push(block('repeatingList'));
        blocks.push('-');
        ...
        blocks.push(block('doSendNetRequest'));
        blocks.push('=');
        blocks.push(block('promptCreateBlock'));
        blocks.push('=');
        blocks.push('=');
    }
    if (category == 'popups' || category == 'mod') {
        blocks.push(block('doShowPopup'));
        blocks.push(block('doShowErrorPopup'));
        blocks.push('-');
        blocks.push(watcherToggle('lastPromptAnswer'));
        blocks.push(block('lastPromptAnswer'));
        blocks.push(watcherToggle('hasLastPromptAnswer'));
        blocks.push(block('hasLastPromptAnswer'));
        blocks.push('-');
        blocks.push(block('doPromptYesOrNo'));
        blocks.push(block('doPromptYesOrNoCustomButtons'));
        blocks.push('-');
        blocks.push(block('doPromptText'));
    }
    if (category == 'mod') { blocks.push('='); blocks.push('='); }
    if (category == 'blocks' || category == 'mod') {
        ...
        blocks.push('-');
        blocks.push(block('getAllBlocks'));
    }
    if (category == 'mod') { blocks.push('='); blocks.push('='); }
    if (category == 'debugging' || category == 'mod') {
        blocks.push(block('receiveUserEdit'));
        blocks.push(block('reportEnvironment'));
        blocks.push('-');
        blocks.push(block('doLogToConsole'));
        blocks.push(block('doWarnToConsole'));
    }
...
SpriteMorph.prototype.freshPalette = function (category) {
    ...
    if (!ide || !ide.config.noOwnBlocks) {
        ...
        makeButton.hint = localize('create a block') + '...';
